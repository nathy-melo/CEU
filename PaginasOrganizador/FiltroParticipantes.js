...
(function () {
    ...
    window.filtroParticipantesConfig = {
    ...
    /**
     * Inicializa o sistema de filtro para participantes
     * @param {string} tabelaId - ID da tabela de participantes
     */
    window.inicializarFiltroParticipantes = function (tabelaId) {
        const config = window.filtroParticipantesConfig;
        config.tabelaId = tabelaId;

        const botaoFiltrar = document.getElementById('btn-filtrar-participantes');
        ...
        const painelAntigo = document.getElementById('filtro-container-participantes');
        if (painelAntigo) {
            painelAntigo.remove();
        }
        ...
        if (!config.documentClickHandler) {
            config.documentClickHandler = function (e) {
                const painel = document.getElementById('filtro-container-participantes');
                const botao = document.getElementById('btn-filtrar-participantes');
    ...
    function salvarDadosOriginais(tabelaId) {
        ...
        const config = window.filtroParticipantesConfig;
        config.dadosOriginais = Array.from(linhas).map((tr, index) => {
            const dados = {
                elemento: tr.cloneNode(true),
                nome: tr.querySelector('td:nth-child(2)')?.textContent.trim() || '',
                cpf: tr.getAttribute('data-cpf') || '',
                dataInscricao: tr.querySelector('td:nth-child(2)')?.textContent.match(/Inscrito em: (.*)/)?.[1] || '',
                presenca: tr.querySelector('td:nth-child(4)')?.textContent.trim() || '',
                certificado: tr.querySelector('td:nth-child(4)')?.textContent.trim() || ''
            };
    ...
    function criarPainelFiltro() {
        ...
        painel.id = 'filtro-container-participantes';
