        ...
        <div class="container" id="eventos-container">
            <?php if ($res && mysqli_num_rows($res) > 0): ?>
                <?php while ($ev = mysqli_fetch_assoc($res)):
                    ...
                    // Data de conclusão para paginação
                    $dataConclusaoISO = date('Y-m-d', strtotime($ev['conclusao']));
                ?>
                    <a class="botao CaixaDoEvento"
                        ...
                        data-data-fim="<?= $dataConclusaoISO ?>"
                        ...
                        <div class="AcoesFlutuantes">
                            <button type="button" class="BotaoAcaoCard BotaoInscreverCard botao" title="Inscrever-se" aria-label="Inscrever"
                                data-cod="<?= (int)$ev['cod_evento'] ?>" data-inscrito="0">
                        ...
                        <div class="EventoInfo">
                            <ul class="evento-info-list" aria-label="Informações do evento">
                                ...
                                <li class="evento-info-item">
                                    ...
                                    <span class="evento-info-texto"><span class="evento-info-label">Certificado:</span> <?= $ev['certificado'] == 1 ? 'Sim' : 'Não' ?></span>
                                </li>
                            </ul>
                        </div>
                    </a>
    ...
    <script>
        ...
        function renderizarFavoritos() {
            ...
            favoritosDados.forEach(ev => {
                ...
                a.href = `ContainerOrganizador.php?pagina=evento&id=${ev.cod_evento}`;
            ...
            // Adicionar listeners nos botões de inscrição dos favoritos
            setTimeout(() => {
        ...
        function adicionarListenersDeClique() {
            ...
            if (window.listenerCliqueOrganizador) {
                document.removeEventListener('click', window.listenerCliqueOrganizador, true);
            }

            // Criar novo listener
            window.listenerCliqueOrganizador = async function(e) {
            ...
            document.addEventListener('click', window.listenerCliqueOrganizador, true);
        }
