...
class ProcessadorTemplate
{
    ...
    /**
     * Valida e processa os dados antes de substitui-los no template.
     * Aplica regras específicas:
     * - TipoCertificado: aceita apenas "ensino", "extensão", "pesquisa"; caso contrário, deixa vazio
     * - Categoria: aceita qualquer valor, mas em minúsculo
     * - NumeroCPF: formata como 123.456.789-01
     */
    private function processarDados(array $dados): array
    {
        ...
        foreach ($dados as $chave => $valor) {
            if ($chave === 'TipoCertificado') {
                // Validação: aceita apenas ensino, extensão ou pesquisa
                $valorLower = strtolower(trim((string)$valor));
                // Normaliza "extensao" para "extensão"
                if ($valorLower === 'extensao') {
                    $valorLower = 'extensão';
                }
                if (in_array($valorLower, ['ensino', 'extensão', 'pesquisa'], true)) {
                    // Capitaliza a primeira letra
                    $dadosProcessados[$chave] = ucfirst($valorLower);
                } else {
                    // Se não for um dos valores válidos, deixa vazio
                    $dadosProcessados[$chave] = '';
                }
            } elseif ($chave === 'Categoria') {
                // Categoria: aceita qualquer valor, mas em minúsculo
                $dadosProcessados[$chave] = strtolower(trim((string)$valor));
            } elseif ($chave === 'NumeroCPF') {
                // Formata CPF: remove caracteres especiais e formata como 123.456.789-01
                $cpf = preg_replace('/\D/', '', (string)$valor);
                if (strlen($cpf) === 11) {
                    $dadosProcessados[$chave] = substr($cpf, 0, 3) . '.' . substr($cpf, 3, 3) . '.' . substr($cpf, 6, 3) . '-' . substr($cpf, 9, 2);
                } else {
                    $dadosProcessados[$chave] = $valor;
                }
            } else {
                $dadosProcessados[$chave] = $valor;
            }
    ...
    {
        // Processa e valida os dados antes de substituir
        $dados = $this->processarDados($dados);
    ...
    /**
     * Aguarda a conclusão da conversão (com timeout).
     * Retorna true se o arquivo foi criado dentro do timeout.
     */
    private function aguardarConversao(string $arquivoEsperado, int $timeoutSegundos = 30): bool
    {
        $inicio = time();
        $intervalo = 100000; // 100ms em microsegundos
        
        while ((time() - $inicio) < $timeoutSegundos) {
            if (file_exists($arquivoEsperado) && filesize($arquivoEsperado) > 0) {
                return true;
            }
            usleep($intervalo);
        }
        
        return file_exists($arquivoEsperado) && filesize($arquivoEsperado) > 0;
    }
    ...
    private function converterViaSofficeBackgroundComAspiracao(string $entrada, string $saidaPdf, ?string $sofficePath = null): array
    {
        ...
        // Windows: usar START /B para detach
        if (stripos(PHP_OS, 'WIN') === 0) {
            $cmd = 'START /B "" ' . $cmd;
            popen($cmd, 'r');
        } else {
            // Linux/Mac: adicionar & para background
            $cmd .= ' > /dev/null 2>&1 &';
            popen($cmd, 'r');
        }
    ...
    /** Pipeline em background (não-bloqueante): docx/pptx/odt/odp/fodt/fodp -> PDF */
    public function gerarPdfDeModeloBackground(string $templatePath, array $dados, string $saidaPdf, ?string $workdir = null): array
    {
        $ext = strtolower(pathinfo($templatePath, PATHINFO_EXTENSION));
        $work = $workdir ?: sys_get_temp_dir();
        $tmp = rtrim($work, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'cert-model-' . uniqid() . '.' . $ext;
        ...
        return $this->converterViaSofficeBackgroundComAspiracao($tmp, $saidaPdf, $this->sofficePath);
    }
}
